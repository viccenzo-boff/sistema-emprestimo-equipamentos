...
| Atalhos | Na **área de trabalho pública** (`C:\Users\Public\Desktop`) **só o Painel**; `reiniciar.cmd` e `atualizar-copia-para-consulta.cmd` ficam em `scripts/implantacao/` | Aparecem para todo usuário do PC; a área de trabalho do usuário pode estar no OneDrive. Os dois `.cmd` saíram da área de trabalho por decisão do dono ao ver a primeira instalação: "eu explico onde acessar no dia" |
...
| Node e Git | **O instalador instala pelo `winget` o que faltar** (e atualiza o Node abaixo de 24) | Pedido do dono depois da primeira execução: o PC da coordenação não tem nada instalado, e o guia não pode depender de a pessoa acertar a versão |
